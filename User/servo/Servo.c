...
/**
  ...
  */
void Servo_open(int dir1)
{
	TIM_SetCompare1(TIM_SERVO,dir1);
	TIM_SetCompare2(TIM_SERVO,dir1);
	TIM_SetCompare3(TIM_SERVO,dir1);
}
void Servo_close(int dir2)
{
	TIM_SetCompare1(TIM_SERVO,dir2);
	TIM_SetCompare2(TIM_SERVO,dir2);
	TIM_SetCompare3(TIM_SERVO,dir2);
}
...
int Turn(int c[5],int P,int D)
{
	...
	/*if(c[1] > c[2] && c[1] > c[3])
	{
		error = ((float)(c[1]-c[2]))/((float)(c[1]+c[2]));
		command = (Kp*error + Kd * (error-LastE)) + Servo_Mid;
		LastE = error;
		protect_comp(&command);
		return command;
	}
	else if(c[3] > c[2] && c[3] > c[1])
	{
		error = ((float)(c[2]-c[3]))/((float)(c[2]+c[3]));
		command = (Kp*error + Kd * (error-LastE)) + Servo_Mid;
		LastE = error;
		protect_comp(&command);
		return command;
